...
🔸🔸 <b>(только на те дни, когда есть пары)</b>`,
        destination: GetChat(ctx),
        buttons: Markup.keyboard(SETTINGS_COMMANDS.map((settingCommand) => [{ text: settingCommand.text(foundUser) }]))
          .reply_markup
      });
    }
  },
            ...
            .then(() => {
              PushIntoSendingImmediateQueue({
                ...
                buttons: Markup.keyboard(
                  SETTINGS_COMMANDS.map((settingCommand) => [{ text: settingCommand.text(foundUser) }])
                ).reply_markup
              });
            })
            .catch(Logging);
        ...
        .then(async (foundUser) => {
          if (foundUser.waitingForGroupSelection) {
            ...
            mongoDispatcher
              ...
              .then(
                /** @param {} foundGroups */ (foundGroups) => {
                  foundGroups = foundGroups.filter(
                    (group, index, array) =>
                      array.findIndex(
                        (matchingGroup) =>
                  ...
                  if (!foundGroups.length) {
                    ...
                    SaveUser(foundUser, ctx)
                      .then(() => {
                        PushIntoSendingImmediateQueue({
                          text: `Ваша группа успешно установлена: <code>${TGE(
                            foundUser.group.replace(/\&/g, ", ")
                          )}</code>`,
                          destination: GetChat(ctx)
                        });
                      })
                    ...
                    SaveUser(foundUser, ctx)
                      .then(() => {
                        PushIntoSendingImmediateQueue({
                          text: `Группу с названием ${TGE(
                            foundUser.selectingGroupName
                          )} надо уточнить. Просто отправьте мне один из следующих вариантов (скорее всего это название кафедры или направления):\n${foundGroups
                            .map((group) => `<code>${TGE(group.groupSuffix)}</code>`)
                            .join(
                              "\n"
                            )}\n\nЕсли что-то пошло не так, вызовите команду /start ещё раз – выбор группы отменится.`,
                          destination: GetChat(ctx),
...
const SaveUser = (foundUser, ctx) => {
  ...
  return new Promise((resolve, reject) => {
    mongoDispatcher
      ...
      .then((updatingResult) => {
        if (updatingResult.ok) resolve();
        else reject(updatingResult);
      })
      .catch(reject);
  });
};
...
const TelegramSend = (messageData) => {
  const replyKeyboard = Markup.keyboard(
    Chunkify(
      Object.keys(COMMANDS).map((key) => ({ text: COMMANDS[key].description })),
      2
    )
      ...
      if (e?.code === 403) {
        ...
        if (foundUser) {
          ...
          if (indexOfFoundUser) {
            ...
            Logging(
              new Error(
                `Could not deleting user with id ${messageData.destination.id} because of critical bug with finding proper user.`
              ),
              e
            );
          }
          ...
          Logging(
            new Error(
              `Cannot remove user with id ${messageData.destination.id} because they're not in out users' list!`
            ),
            e
          );
        }
        ...
        if (foundUser) {
          mongoDispatcher
            .callDB()
            .then((DB) =>
              DB.collection("telegram_users").updateOne(
          ...
          Logging(
            new Error(
              `Cannot update user with id ${messageData.destination.id} because they're not in out users' list!`
            ),
            e
          );
        }
...
const ImmediateSendingQueueProcedure = (iMessageData) => {
  ...
  TelegramSend(messageData).catch(
    /** @param {TelegramError} e */ (e) => {
      if (e && e.code === 429) {
        if (typeof e.parameters?.retry_after == "number")
          setTimeout(
            () => ImmediateSendingQueueProcedure(messageData),
            e.parameters?.retry_after * 1e3 + Math.random() * 2e3
          );
        else setTimeout(() => ImmediateSendingQueueProcedure(messageData), 2e3);
      } /* else
  ...
  TelegramSend(messageData).catch(
    /** @param {TelegramError} e */ (e) => {
      if (e && e.code === 429) {
        if (typeof e.parameters?.retry_after == "number")
          setTimeout(
            () => MailingSendingQueueProcedure(messageData),
            e.parameters?.retry_after * 1e3 + Math.random() * 5e3
          );
        else setTimeout(() => MailingSendingQueueProcedure(messageData), 5e3);
      } /* else 
			Logging(new Error(`Error on sending to ${messageData.destination.id}`), e); */
    }
  );
...
telegraf.start(
  /** @param {import("telegraf").Context} ctx */ (ctx) => {
    const foundUser = USERS.find((user) => user.id === GetChat(ctx).id);
...
telegraf.on("text", (ctx) => {
  ...
  if (commandMatch && commandMatch[1]) {
    if (COMMANDS[commandMatch[1]]) {
      if (typeof COMMANDS[commandMatch[1]].caller == "function") return COMMANDS[commandMatch[1]].caller(ctx);
  ...
  if (foundUser && foundUser.waitingForGroupSelection) {
    ...
    if (groupCommandHandler) return groupCommandHandler.caller(ctx);
    else
      return PushIntoSendingImmediateQueue({
  ...
  if (foundUser && foundUser.waitingForTextForSettings) {
    ...
    if (settingsCommandHandler) return settingsCommandHandler.caller(ctx);
    else
      return PushIntoSendingImmediateQueue({
        text: "Не понял тебя. Если долго ничего не получается, попробуй команду /start. Или посмотри описание бота.",
        destination: GetChat(ctx)
  ...
  if (COMMANDS_ALIASES[commandAlias]) {
    if (typeof COMMANDS_ALIASES[commandAlias].caller == "function") return COMMANDS_ALIASES[commandAlias].caller(ctx);
