 ...
 * @returns {String}
 */
const BuildOption = (iGroup, iDay, iOptions, iLessonPosition, iWeek) => iOptions.filter((option) => {
	...
	return false;
}).map((option, optionIndex) => BuildOptionLayout(iGroup, iDay, option, iLessonPosition, optionIndex > 0)).join("\n").trim();
