 ...
 */
const GlobalSafeParseXLSX = (XLSXData) =>
 ...
 */
const GetLinkToFiles = () =>
  Promise.all(
    SCHEDULE_PAGES.map((schedulePage, schedulePageIndex) =>
      fetch(schedulePage.baseUrl, BuildFetchOptions(schedulePage.cookie))
        .then((res) => {
          if (res.status === 200) return res.text();
          else
            return Promise.reject(
              new Error(`Error on getting ${schedulePage.baseUrl}\nStatus code ${res.status} ${res.statusText}`)
            );
        })
        .then((page) => {
          if (DEV) writeFile(`./out/mirea-ru-${schedulePage.type}-${schedulePageIndex}.html`, page).catch(() => {});
          ...
          if (schedulePage.type === "main") {
            const XLSX_FILES_HREF_REGEXP =
              /<[\w]+(\s+[\w\-]+(\=("|')[^"']*(\3))?)*\sclass="([^"]*\s+)*uk-link-toggle(\s+[^"]*)*"(\s+[\w\-]+(\=("|')[^"']*(\9))?)*\s+href="([^"]+)"(\s+[\w\-]+(\=("|')[^"']*(\14))?)*>/gi;
            const XLSX_FILES_HREF_REGEXP_GROUP_INDEX = 11;

            const XLSX_FILES_COURSE_REGEXP =
              /<[\w]+(\s+[\w\-]+(\=("|')[^"']*(\3))?)*\sclass="([^"]*\s+)*uk-link-heading uk-margin-small-top(\s+[^"]*)*"(\s+[\w\-]+(\=("|')[^"']*(\9))?)*>([^<]+)/gi;
            const XLSX_FILES_COURSE_REGEXP_GROUP_INDEX = 11;

            /** `"Бакалавриат, магистратура, etc."` */
            const unitsNames = parsedMainPage
              .querySelectorAll(".schedule .uk-active a")
              .map((anchor) => anchor.innerText);
            const unitsHTMLBlocks = parsedMainPage
              .querySelector(".uk-switcher")
              .childNodes.filter((child) => child.nodeType === 1);

            unitsHTMLBlocks.forEach(
              /** @param {import("node-html-parser").HTMLElement} unitHTMLBlock */ (unitHTMLBlock, unitIndex) => {
                if (!unitHTMLBlock || unitHTMLBlock.nodeType !== 1) return;

                unitHTMLBlock
                  .querySelectorAll(".uk-card.slider_ads.uk-card-body.uk-card-small")
                  .forEach((instituteCard) => {
                    ...
                    instituteCard.outerHTML.split("<hr").forEach((subInstitute) => {
                      ...
                      const usualTableCardsLayout = subInstitute
                        .split("Расписание занятий")?.[1]
                        ?.split(/расписание/i)[0];
                      ...
                      allXLSXDefinitions.push(
                        ...subInstituteLinks.map(
                          /** @return {XLSXFileDefinition} */ (link, index) => ({
                            remoteFile: link,
                            unitName: instituteName,
                            unitCourse: `${unitsNames[unitIndex]}${
                              subInstituteCourses[index] ? ", " + subInstituteCourses[index] : ""
                            }`
                          })
                        )
                      );
            ...
            const NEWS_HEADER_TARGET_REGEXP =
              /Расписани.*семестра.*\d{2,4}-?\d{2,4}.*г(?:ода|.)?\s(\d+(?:-\d+)?\sкурс(?:.*формы)?)/i;
            const NEWS_HEADER_TARGET_GROUP_INDEX = 1;
            const FRYAZINO_UNIT_NAME = "Филиал Фрязино";

            const newsHeaders = parsedMainPage
              .querySelectorAll(".news-list .news-item-header")
              .filter((header) => NEWS_HEADER_TARGET_REGEXP.test(header.innerText));
            ...
            return Promise.all(
              scheduleNewsTargets.map((scheduleNewsTarget, scheduleNewsTargetIndex) =>
 ...
 */
const GetTablesFiles = (allXLSXDefinitions) =>
  new Promise((resolve, reject) => {
    ...
    const LocalRecurion = (iIndex) => {
      ...
      if (!gettingFileProps) return resolve(allXLSXFilesData);

      if (DEV) Logging(`Getting file ${encodeURI(gettingFileProps.remoteFile)}`);

      fetch(encodeURI(gettingFileProps.remoteFile), BuildFetchOptions())
        .then((res) => {
        ...
        .then((fileData) => {
          ...
          if (DEV)
            writeFile(
              `./data/${new URL(gettingFileProps.remoteFile).pathname
 ...
 */
const BuildGlobalSchedule = (iXLSXFilesData) =>
  new Promise((resolve) => {
    /**
     * @param {number} index
     * @returns {void}
      ...
      GlobalSafeParseXLSX(XLSXFileData.fileData)
        .then((workSheetsFromFile) =>
          workSheetsFromFile.map((tableSheet) => {
            ...
            const indexesOfCellsWithGroupNames = lineWithGroups
              .map((cell, index) => {
                if (typeof cell !== "string") return null;

                if (/^[\wа-яё]{4}-\d{2}-\d{2}/i.test(cell?.trim?.())) return index;
                else return null;
              })
            ...
            const lessonsTimes = lessonsByDays.map((lessonsInDay, dayIndex) => {
              ...
              const timesForDay = Array.from({ length: lessonsInDay }, (_, lessonTimeIndexInDay) => {
                const lessonIndex =
                  INDEX_OF_LINE_WITH_GROUPS_NAMES + 2 + skipFromFirstLessonOnWeek + lessonTimeIndexInDay;
                const lessonTimeStart = tableData[lessonIndex][2];
                const lessonTimeEnd = tableData[lessonIndex][3];

                if (
                  lessonTimeStart &&
                  typeof lessonTimeStart == "string" &&
                  lessonTimeEnd &&
                  typeof lessonTimeEnd == "string"
                )
                  return `${lessonTimeStart.replace(/(\d+)(:|-)(\d+)/, "$1:$3")} – ${lessonTimeEnd.replace(
                    /(\d+)(:|-)(\d+)/,
                    "$1:$3"
                  )}`;
              ...
              certainGroupTable.forEach((lessonOption, lessonOptionIndex) => {
                ...
                if (splittedLesson.name && splittedLesson.name instanceof Array) {
                  ...
                  if (splittedLesson.name.length === 2 && /(\d+\s*п\/г)(,\1)*/i.test(splittedLesson.name[1]))
                    splittedLesson.name[0] = `${splittedLesson.name[0]} ${splittedLesson.name
                      .pop()
                      .replace(/([\d,])/g, "$1 ")}`.trim();
                  ...
                  splittedLesson.name.forEach((optionName, optionIndex) => {
                    ...
                    if (weeksMatch && weeksMatch[1]) weeks = weeksMatch[1];
                    else weeks = null;
                    ...
                    if (!weeks) {
                      ...
                      if (weeksMatch && weeksMatch[1] && weeksMatch[2] && weeksMatch[3]) {
                        ...
                        for (let i = startingWeek; i <= endingWeek; i += 2) weeksArr.push(i);
                    ...
                    if (weeksExclude.length) {
                      if (!weeks) {
                        ...
                        for (let i = 1 + (lessonOptionIndex % 2); i <= 16; i += 2) {
                          if (!weeksExclude.includes(i)) weeks.push(i);
                        }
                    ...
                    formedLesson.push({
                      ...
                      type: splittedLesson.type
                        ? splittedLesson.type[optionIndex] || splittedLesson.type[0] || null
                        : null,
                      tutor: splittedLesson.tutor
                        ? splittedLesson.tutor[optionIndex] || splittedLesson.tutor[0] || null
                        : null,
                      place: splittedLesson.place
                        ? splittedLesson.place[optionIndex] || splittedLesson.place[0] || null
                        : null,
                      link: splittedLesson.link
                        ? splittedLesson.link[optionIndex] || splittedLesson.link[optionIndex - 1] || null
                        : null
                    });
                  });
                }

                if (lessonOptionIndex % 2) schedule[dayOfWeek].even.push(formedLesson);
                else schedule[dayOfWeek].odd.push(formedLesson);
              });
...
GetLinkToFiles()
  ...
  .then(() => {
    if (DEV) {
      ...
      writeFile("./out/global-schedule.json", JSON.stringify(GLOBAL_SCHEDULE, false, "\t"));
    }
    ...
    return mongoDispatcher.callDB().then((DB) => {
      ...
      return STUDY_GROUPS_COLLECTION.insertMany(GLOBAL_SCHEDULE)
        .then(
          () =>
            new Promise((resolveClearingPrevious) => {
              /**
               * @param {number} clearingGroupIndex
               */
              const LocalRecursionRemove = async (clearingGroupIndex) => {
                ...
                if (successfullyDeletedSameButPrevious) return LocalRecursionRemove(clearingGroupIndex + 1);
                ...
                STUDY_GROUPS_COLLECTION.deleteMany({
                  ...
                  .catch(() => {})
                  .finally(() => LocalRecursionRemove(clearingGroupIndex + 1));
              };

              LocalRecursionRemove(0);
            })
        )
        .then(() =>
          STUDY_GROUPS_COLLECTION.aggregate([
            ...
            .then(
              ...
              (aggregatedGroups) =>
                Promise.all(
                  aggregatedGroups.map((aggregatedGroup) => {
                    ...
                    if (uniqueSuffixes.length === aggregatedGroup.groupSuffix.length) return Promise.resolve();
                    ...
                    return STUDY_GROUPS_COLLECTION.find({
                      ...
                      .toArray()
                      .then((foundGroups) => {
                        const uniqueFoundGroups = foundGroups.filter(
                          (foundGroup, index, array) =>
                            index ===
                            array.findIndex(
                              (matchingGroup) =>
        ...
        .then(() =>
          PARAMS_COLLECTION.findOneAndUpdate(
